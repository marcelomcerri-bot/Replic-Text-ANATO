...
const subtopicsData: SubtopicImages[] = [
  {
...
async function validateUrl(url: string): Promise<{ valid: boolean; size?: number; error?: string }> {
...
async function validateAndDownloadImages() {
  ...
  for (const subtopic of subtopicsData) {
    ...
    const subtopicDir = path.join(baseDir, subtopic.subtopico.toLowerCase().replace(/\s+/g, '-').replace(/á/g, 'a').replace(/é/g, 'e').replace(/í/g, 'i').replace(/ó/g, 'o'))
    
    if (!fs.existsSync(subtopicDir)) {
      fs.mkdirSync(subtopicDir, { recursive: true })
    }

    const subtopicLog: SubtopicImages = {
    ...
    for (const image of subtopic.images) {
      ...
      if (validation.valid) {
        ...
        const fileName = `${image.type}-${subtopic.subtopico.toLowerCase().replace(/\s+/g, '-').replace(/á/g, 'a').replace(/é/g, 'e').replace(/í/g, 'i').replace(/ó/g, 'o')}.png`
